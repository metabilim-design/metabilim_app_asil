// lib/pages/coach/homework_flow/continue_select_materials_page.dart
...
class ContinueSelectMaterialsPage extends StatefulWidget {
  ...
  const ContinueSelectMaterialsPage({
    Key? key,
  ...
  @override
  _ContinueSelectMaterialsPageState createState() => _ContinueSelectMaterialsPageState();
}

class _ContinueSelectMaterialsPageState extends State<ContinueSelectMaterialsPage> {
  ...
  void initState() {
    super.initState();
    _fetchMaterialsAndPreselect();
  }

  Future<void> _fetchMaterialsAndPreselect() async {
    ...
    try {
      ...
      widget.schedule.values.forEach((slots) {
        for (var slot in slots) {
          ...
          // Dersleri toplarken "Boş Etüt"leri hariç tutuyoruz.
      ...
      final List<String> previouslySelectedIds = List<String>.from(previousData?['materials'] ?? []);
      ...
      for (String fullLessonName in uniqueLessons) {
        ...
        for (var doc in booksSnapshot.docs) {
          ...
          items.add(MaterialItem(
              ...
              isSelected: previouslySelectedIds.contains(doc.id)
          ));
        }
      ...
      print('Materyal çekerken hata: $e');
    } finally {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Materyalleri Güncelle'),
      ),
      body: _isLoading
